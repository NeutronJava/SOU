...
hs.buffer = frame
local status, err
local frameDuration = 0.05 * sleep_ticks
local startTime = os.clock()
local frameCount = 0

while true do
    local expectedTime = startTime + frameCount * frameDuration
    local now = os.clock()

    -- Sync audio update
    if wc then
        pcall(wc.update, wc, frameDuration)
    end

    -- Process a frame
    local status, err = pcall(readFrame)
    if not status then
    ...
    -- Wait just enough to match the expected framerate
    local delay = expectedTime + frameDuration - os.clock()
    if delay > 0 then
        sleep(delay)
    end
